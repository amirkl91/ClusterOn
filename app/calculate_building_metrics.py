...
def get_all_building_metrics(buildings: gpd.geodataframe, height_column_name=None):
    ...
    buildings["num_of_neighbours"] = momepy.neighbors(
        tessellation, contiguity, weighted=True
    )
    return buildings
...
if __name__ == "__main__":
    ...
    gdf = gdf[["סיווג_מרקמים_12042024", "geometry"]]

    buildings_gdf = get_all_building_metrics(gdf)
    print(buildings_gdf.columns)
    buildings_gdf.to_file(
        "buildings_with_classification.gpkg", layer="streets", driver="GPKG"
    )
